...
class Parser {
     ...
     */
    fun readConversionRates(path: String): Map<String, String?> {
        ...
        return rates.map { rate ->
            val originalDateSplit = rate.key.split("-")
            val newDate = "${originalDateSplit[2]}/${originalDateSplit[1]}/${originalDateSplit[0]}"
            newDate to rate.value["EUR"].toString()
